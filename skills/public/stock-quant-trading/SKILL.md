...
description: 股票分析、量化选股、个股分析、技术分析、买卖信号、持仓/观望建议、行业轮动。用于A股/港股量化交易，支持个股深度分析（实时行情/技术指标/财务/研报/舆情/综合研判六维度）和全市场量化选股（8因子Alpha模型/多因子筛选/组合优化/风控建议）。所有数据通过 stock-data-crawl API + Python 脚本获取，禁止 web_search/web_fetch。
allowed-tools:
  - read_file
  - write_file
  - bash
...
`signals.json` 中 `signal` 字段到报告中文标签的映射：

| 信号值 | 中文标签 | 判定条件 |
|--------|---------|----------|
| strong_buy | 强烈买入 | Alpha ≥ 80，未触发降级 |
| buy | 买入 | Alpha ≥ 65，或 strong_buy 被风险降级后 |
| hold | **观望** | Alpha ≥ 35，或 buy 被风险标签/技术破位降级 |
| sell | 卖出 | 技术破位 ≥4 项直接触发 |
| strong_sell | 强烈卖出 | Alpha < 20 |

### 信号降级规则

`alpha_model.py` 在初始信号判定后执行两级降级：

1. **风险标签降级**：`risk_flags` 包含 `PE极端高` 或 `经营现金流为负` → 降 1 级（strong_buy→buy, buy→观望）。**注意**：仅 `经营现金流为负` 触发降级，`自由现金流为负` 不触发。
2. **技术破位降级**：命中 ≥2 项破位指标（跌破MA20/MA60、MA5/20死叉、RSI弱势、MACD死叉、短期急跌）→ 降 1 级；≥3 项降 2 级；≥4 项直接判 sell

> `signals.json` 中 `downgraded_from` 字段记录降级来源（未降级则为 `null`）。报告中被降级的信号用 `↓` 标记：`买入↓` = 从 strong_buy 降来，`观望↓` = 从 buy 降来。
...
>
> 1. **禁止用总结/检查清单替代报告** — 不要输出 "✅ 已完成" 表格或概述替代实际报告内容
> 2. **6 个维度必须逐一完整呈现** — 每维度必须包含实际数据表格和分析文字，不可跳过或用 "详见上述" 代过
> 3. **报告优先在对话中呈现** — 必须在对话回复中直接呈现给用户。仅在模型输出超出单次响应 token 上限时，使用下方 Report Delivery Strategy 中定义的拆分/文件降级策略
> 4. **报告送达后允许 1 条确认消息** — 分段输出完成后，可附加一条简短确认消息（如 "分析报告 6 维度全部完成"），但不包含重复的总结或交付物清单

##### A7b: Report Delivery Strategy（场景 A 长报告处理）

> 单股 6 维度完整报告 + 多张数据表格，总输出可能超过单次模型响应的 token 上限。按以下优先级处理：

**优先级 1: 分段输出（推荐）**

将 6 个维度拆分为 2-3 段逐条消息输出。拆分规则：

| 段 | 包含维度 | 结束标记 |
|:--:|----------|----------|
| 1 | 实时行情 + 技术分析 | `⏎（第 1/3 段，续见下条）` |
| 2 | 财务分析 + 研报分析 | `⏎（第 2/3 段，续见下条）` |
| 3 | 舆情分析 + 综合研判（含免责声明） | 无标记，以 `免责声明` 结尾 |

- 每段开头不重复标题 `# 单股深度分析报告：{股票名称}({代码})`，改为 `#### 续：单股深度分析报告：{股票名称}({代码})`（第 1 段除外，仍用 `#` 级标题）
- 分段完成后允许附加 1 条简短确认消息，但不得包含重复的总结或交付物清单

**优先级 2: 文件降级（仅当分段仍无法完整输出时）**

若分段输出后仍有个别维度因 token 限制无法送达：

1. 将完整报告写入 `/mnt/user-data/workspace/report_A_{code}.md`
2. 在对话中输出简要提示：`完整报告已写入 /mnt/user-data/workspace/report_A_{code}.md，包含 6 个维度。以下为报告摘要：`
3. 在对话中输出精简版（每维度 2-3 行关键结论，不要求完整表格）
4. 精简版以 `免责声明` 结尾

> **严禁**：在未尝试分段输出前直接降级为文件模式。优先使用分段输出。
...
> 2. **6 个章节必须逐一完整呈现** — 每章必须包含实际数据表格和分析文字，不可跳过或用 "详见上述" 代过
> 3. **详细持仓表必须列出全部股票** — Chapter 4 必须完整列出 portfolio.json 中所有 allocations[]，不可只列前 N 只
> 4. **报告优先在对话中呈现** — 必须在对话回复中直接呈现给用户。仅在模型输出超出单次响应 token 上限时，使用 [Report Delivery Strategy](#report-delivery-strategy) 中定义的拆分/文件降级策略
> 5. **报告送达后允许 1 条确认消息** — 分段输出完成后，可附加一条简短确认消息说明报告已全部送达（如 "报告 6 章全部完成"），但不包含重复的总结或交付物清单
...
> - 报告以 `# 📊 全市场量化选股 — 完整报告` 开头
>
> **输出长度管理**：报告包含 20 只股票详细持仓 + 因子分 + 图表 URL，总长度可能超过 5000 字符。如评估发现单次响应无法完整输出全部 6 章，优先采用下方 [Report Delivery Strategy](#report-delivery-strategy) 中定义的拆分策略，而非压缩内容。
...
> **信号列使用 [Signal Labels](#signal-labels) 中定义的中文标签**。被降级的信号追加 `↓` 标记：检查 `signals.json` 中同 code 的 `downgraded_from` 字段 — 若为 `"strong_buy"` 且当前 `signal == "buy"` → 显示 `买入↓`；若为 `"buy"` 且当前 `signal == "hold"` → 显示 `观望↓`。未降级则不加标记。

> **风险标记列**：从 `signals.json` 中按 code 匹配 `risk_flags`，**原文输出不缩写**。区分 `经营现金流为负` 和 `自由现金流为负`（仅前者触发信号降级）。
...
|:-:|:----:|:----:|:----:|:----:|:-----:|:----:|:-----:|:--------:|:--------:|
| 1 | XXXXXX | XX | XX | XXXX | **XX.XX** | 买入 | X.XX% | ¥XX,XXX | 标签 |

> 若持仓中有 `signal == "buy"` 和 `signal == "hold"` 两种信号，先列买入信号（含 `买入↓`），再列观望信号（含 `观望↓`），两组之间用分隔标题区分。

展示完所有持股后，再展示**卖出信号的股票**（从 `signals.json` 中筛选 `signal == "sell"` 或 `signal == "strong_sell"`），提示用户规避风险。
...
> 6 章节完整报告 + 20 只股票详细持仓表 + 多张内嵌图表，总输出可能超过单次模型响应的 token 上限。按以下优先级处理：

### 优先级 1: 分段输出（推荐）

将 6 个章节拆分为 2-3 段逐条消息输出。拆分规则：

| 段 | 包含章节 | 结束标记 |
|:--:|----------|----------|
| 1 | 一市场总览 + 二因子权重配置 | `⏎（第 1/3 段，续见下条）` |
| 2 | 三组合配置方案 + 四组合详细持仓 | `⏎（第 2/3 段，续见下条）` |
| 3 | 五风控参考 + 六总结（含免责声明） | 无标记，以 `免责声明` 结尾 |

- 每段开头不重复标题 `# 📊 全市场量化选股 — 完整报告`，改为 `#### 续：全市场量化选股 — 完整报告`（第 1 段除外，仍用 `#` 级标题）
- 第 4 段（可选）：如果详细持仓表过长（20 只股票无法塞入 1 段），将持仓表拆成 2 段，总段数变为 4
- 分段完成后允许附加 1 条简短确认消息，但不得包含重复的总结或交付物清单

### 优先级 2: 文件降级（仅当分段仍无法完整输出时）

若分段输出后仍有个别章节因 token 限制无法送达：

1. 将完整报告写入 `/mnt/user-data/workspace/report.md`
2. 在对话中输出简要提示：`完整报告已写入 /mnt/user-data/workspace/report.md，包含 6 个章节。以下为报告摘要：`
3. 在对话中输出精简版（每章 2-3 行关键结论，不要求完整表格）
4. 精简版以 `免责声明` 结尾
...
- 单股分析 → 读取 `indicators.json` + `finance.json` + `report.json`，按场景 A7 的 6 维度模板在回复中呈现报告，若报告过长则按场景 A7 的 Report Delivery Strategy 分段输出或文件降级
...
- **报告以 `免责声明` 结尾即止** — 不要在其后追加 "B6 全部完成"、"✅ 已完成交付物" 等总结消息。分段输出时，最后一段以 `免责声明` 结尾；允许在最后一段之后发送 1 条简短确认消息（如 "报告 6 章全部完成"），但不应包含重复的总结或交付物清单
- **覆盖所有持股**：详细持仓表必须列出组合中全部股票，禁止只列前 10 只或 "等 N 只" 等缩写
- **分段输出标记**：多段输出时，每段结尾标 `⏎（第 X/3 段，续见下条）`，最后一段不标
